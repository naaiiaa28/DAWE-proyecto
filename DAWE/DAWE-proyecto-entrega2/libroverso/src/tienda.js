...
export function getTipoProducto(producto) {
  if (producto.autor !== undefined)            return 'novela'
  if (producto.campo !== undefined)            return 'ciencia'
  if (producto.editor !== undefined)           return 'ensayo'
  if (producto.edadRecomendada !== undefined)  return 'infantil'
  if (producto.ilustrador !== undefined)       return 'comic'
  return ''
}

export function getExtraValor(producto) {
  if (producto.autor !== undefined)            return producto.autor
  if (producto.campo !== undefined)            return producto.campo
  if (producto.editor !== undefined)           return producto.editor
  if (producto.edadRecomendada !== undefined)  return producto.edadRecomendada
  if (producto.ilustrador !== undefined)       return producto.ilustrador
  return ''
}
